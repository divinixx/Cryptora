...
from app.schemas import UserCreate, UserResponse, UserWithNotes, NoteCreate, NoteUpdate, NoteResponse, NoteWithDecrypted, LoginRequest, LoginResponse, ShareNoteRequest, SharedNoteResponse, SharedNotePublicView
from app.services import UserService, NoteService, SharedNoteService
...
@router.post("/{alias}/notes/{note_id}/share", response_model=SharedNoteResponse)
async def share_note(
    alias: str, 
    note_id: int, 
    password: str, 
    request: Request,
    share_data: ShareNoteRequest = None,
    db: Session = Depends(get_db)
) -> SharedNoteResponse:
    """Create a shareable link for a note"""
    user_service = UserService(db)
    note_service = NoteService(db)
    shared_service = SharedNoteService(db)
    ...
    expires_in_hours = share_data.expires_in_hours if share_data else None
    shared_note = shared_service.create_shared_note(note, password, expires_in_hours)
    
    # Build share URL
    base_url = str(request.base_url).rstrip('/')
    share_url = f"{base_url}/shared/{shared_note.share_token}"
    
    return SharedNoteResponse(
        id=shared_note.id,
        note_id=shared_note.note_id,
        share_token=shared_note.share_token,
        share_url=share_url,
        created_at=shared_note.created_at,
        expires_at=shared_note.expires_at,
        view_count=shared_note.view_count,
        is_active=shared_note.is_active
    )


@router.get("/{alias}/notes/{note_id}/share", response_model=SharedNoteResponse)
async def get_share_status(
    ...
    shared_note = shared_service.get_shared_note_by_note_id(note_id)
    if not shared_note:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Note is not shared")
...
@router.delete("/{alias}/notes/{note_id}/share", status_code=status.HTTP_204_NO_CONTENT)
async def unshare_note(alias: str, note_id: int, password: str, db: Session = Depends(get_db)) -> None:
    ...
    if not shared_service.delete_shared_note(note_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Note is not shared")


@router.get("/shared/{share_token}", response_model=SharedNotePublicView)
async def view_shared_note(share_token: str, db: Session = Depends(get_db)) -> SharedNotePublicView:
    """View a publicly shared note (no authentication required)"""
    shared_service = SharedNoteService(db)
    
    shared_note = shared_service.get_shared_note_by_token(share_token)
    if not shared_note:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Shared note not found or has expired")
    
    note_data = shared_service.view_shared_note(shared_note)
    
    return SharedNotePublicView(
